...
class RandomErasing(object):
    ...
    def __call__(self, img):
        ...
        if self.erasing > self.probability:
            return img
        ...
        for attempt in range(100):
            ...
            if w < img.size()[2] and h < img.size()[1]:
                x1 = int(round(self.tl_x * (img.size()[1] - h)))
                y1 = int(round(self.tl_y * (img.size()[2] - w))) # Changed by baist!
    ...
    def randomize_parameters(self):
        self.erasing = random.uniform(0, 1) < self.probability
